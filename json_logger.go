...
func (jl *JSONLogger) Error(message string) LogInstance {
    return getJSONLog(jl, ERROR, message, jl.pretty)
}
...
func (jl *JSONLogger) SetLogLevel(logLevel string) error {
    ...
    jl.logsEnabled = logLevels

    return nil
}

func (jl *JSONLogger) SetLogsEnabled(logsEnabled map[LogLevel]bool) {
    jl.logsEnabled = logsEnabled
}

func (jl *JSONLogger) SetGoLoggers(
...
func (jl *JSONLogger) CloneTo(targetLogger ChainLogger) {
    targetLogger.SetInternalLogger(jl.logger)
    targetLogger.SetLogsEnabled(jl.logsEnabled)
    targetLogger.SetGoLoggers(jl.logger)
}
