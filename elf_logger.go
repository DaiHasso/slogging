...
func (el *ELFLogger) SetLogLevel(logLevel string) error {
    ...
    el.logsEnabled = logLevels

    return nil
}

func (el *ELFLogger) SetLogsEnabled(logsEnabled map[LogLevel]bool) {
    el.logsEnabled = logsEnabled
}

func (el *ELFLogger) SetLogTarget(
    logTarget LogTarget, otherLogTargets ...LogTarget,
) {
    el.logTarget = logTarget
}

func (el *ELFLogger) SetGoLoggers(
    logger *log.Logger, otherLoggers ...*log.Logger,
) {
    el.logger = logger
}

// CloneTo copies a logger settings from one logger to another.
func (el ELFLogger) CloneTo(targetLogger ChainLogger) {
    targetLogger.SetInternalLogger(el.logger)
    targetLogger.SetLogsEnabled(el.logsEnabled)
    targetLogger.SetGoLoggers(el.logger)
}
